...
const prompts = {
  ...
  async _configPrompt() {
    ...
    const questions = [
      ...
      },
      {
        type: "list",
        name: "userVPCCIDR",
        message: "Select the existing VPC CIDR block you want to connect to your Lodge VPC",
        choices: vpcs.map(vpc => vpc.cidr),
        validate: this._validateCIDR,
        when: (answers) => answers.deployment === "Spin up a new VPC"
      },
    ];
    ...
    if (answers.deployment === "Use an existing VPC") {
      return {...answers, vpc: selectedVpc, subnets}
    } else {
      return answers;
    }
  },
  ...
  _confirmConfigMessage(answers) {
    return (answers.deployment === "Use an existing VPC") ?
      ...
      CIDR to whitelist: ${answers.userVPCCIDR};`
  },
